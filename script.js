...
/*const API_URL = "https://script.google.com/macros/s/AKfycbyMPiXNpcXrzb3eDagjdyYlfZkbykNddnTTvpMkRfWCgT14B7Y3Qk6XBgdXCpbz5TDuGg/exec";*/
const API_URL = "https://script.google.com/macros/s/AKfycbzjTUEpqH84j1mDKHaR4xFo0-KRZtOiPqI7Ycn443iHhGZGCtO3J-CvFXGZzto3vKZyQg/exec";
...
async function showBookedDetails(dateStr) {
    await showCustomAlert("Unavailable", `This date (${dateStr}) is already booked!`);
}

async function submitBooking() {
    // 1 - Confirm booking before proceeding
    const isConfirmed = await showCustomConfirm("Our team will call you back for payment and confirmation.");
    ...
    try {
        ...
        if (result.success) {
            // 2 - Thankful message upon completion
            showToast("Thank you for your booking! Our team will call you back for payment and confirmation.");
            await showCustomAlert("Thank You!", "Thank you for your booking! Our team will call you back for payment and confirmation.");
...
function showCustomConfirm(message) {
    return new Promise((resolve) => {
        ...
        modal.style.display = 'flex';
        // Allow reflow, then apply show class for opacity transition
        setTimeout(() => modal.classList.add('show'), 10);
        ...
        const onOk = (e) => { e.preventDefault(); e.stopPropagation(); cleanup(); resolve(true); };
...
function showCustomAlert(title, message) {
    return new Promise((resolve) => {
        ...
        const onOk = (e) => { e.preventDefault(); e.stopPropagation(); cleanup(); resolve(); };
